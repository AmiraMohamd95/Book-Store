...
public class UserController<SearchResults> {
	...
	public class SignUpListener implements ActionListener {
		...
		public void actionPerformed(ActionEvent arg0) {
			...
			user = (UserModel)manger;
			mainPage.setAccess("user");
			String user_name =  signUp.getUser_name().getText().length()== 0 ? 
				null :"'"+ signUp.getUser_name().getText()+"'"; 
			String last_name = signUp.getLast_name().getText().length()== 0 ?
					null : "'"+signUp.getLast_name().getText()+"'";
			String first_name = signUp.getFirst_name().getText().length()== 0?
					null : "'"+signUp.getFirst_name().getText()+"'";
			String phone_number =signUp.getPhone_number().getText().length()== 0 ? 
					null: "'"+signUp.getPhone_number().getText()+"'";
			String shipping_address = signUp.getShipping_address().getText().length()== 0 ?
					null : "'"+signUp.getShipping_address().getText()+"'";
			String email = signUp.getEmail().getText().length()== 0? 
					null  :"'"+signUp.getEmail().getText()+"'";
			String password =  signUp.getPassword().getText().length()== 0 ?
					null : "'"+ signUp.getPassword().getText() + "'";
			...
			boolean valid = user.signUp( user_name, first_name, last_name,email,password,
					phone_number,shipping_address,"'user'",new ArrayList<String>((Arrays.asList(credit_cards))),new ArrayList<String> (Arrays.asList(expireDates)));
			if(!valid)
			{
	...
	public class ViewCart implements ActionListener {
		...
		public void actionPerformed(ActionEvent arg0) {
			...
			{
				JOptionPane.showMessageDialog(cart.frame,
						"sorry !","Error",
						JOptionPane.ERROR_MESSAGE);
			...
			for(int i = 0 ; i < cartContents.size()-1 ;i++)
			{
				...
				String elemet = "Title: " + book.get(0) + " Price: " +book.get(1);
	...
	private class ModifyListener implements ActionListener {
		...
		public void actionPerformed(ActionEvent arg0) {
			...
			System.out.println("in modifyyyyyy");
			try {
				...
				manager.manger.frame.setVisible(true);
					...
					JOptionPane.showMessageDialog(null,
							"u haven't the rights to modifiy !","Error",
							JOptionPane.ERROR_MESSAGE);
				}
